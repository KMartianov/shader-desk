...
- **Гибкая настройка**: Все параметры настраиваются через JSON-конфигурацию
- **Горячая перезагрузка**: Изменения применяются на лету без перезапуска
- **Модульность**: Поддержка плагинов с различными эффектами
- **Автоматическое управление**: Удобные скрипты для запуска и настройки
...
**Обои не запускаются:**
- Убедитесь, что Wayland композитор поддерживает `wlr-layer-shell`
