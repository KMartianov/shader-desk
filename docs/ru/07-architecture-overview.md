...
```mermaid
flowchart LR
    subgraph ОС [Операционная Система]
        PW(PipeWire / Pulse)
        EV(/dev/input)
    end
...
sequenceDiagram
    ...
    participant S as UNIX Сокет (Буфер ОС)
    participant E as epoll (Ядро)
    participant P as C++ Провайдер
...
stateDiagram-v2
    ...
    state "Уничтожение бракованной копии (v2)\nВывод лога ошибки в терминал" as Discard
